...
class VLESS:
    ...
    vless = VLESS('212.34.140.237', 'gdCDV57RQut64Uzg')

    # Добавляем ключ
    vless.addOrUpdateKey('TEST_VPN_77777')

    # Удаляем ключ
    vless.deleteKey('TEST_VPN_77777')

    # Список активных ключей
    active_keys = vless.activ_list()
    print(active_keys)
    """
    ...
    def __init__(self, ip, password):
        ...
        # ВАЖНО: https, а не http (иначе получите ошибку ProtocolError)
        ...
        if self.con:
            ...
            logger.warning(f'🛑Подключение к панели 3x-ui {self.ip} не произошло (авторизация неуспешна).')
    ...
    def _connect(self):
        ...
        try:
            ...
            if resp.get('success') is True:
                ...
                logger.warning(f'🛑Подключение к панели 3x-ui {self.ip} не произошло, ошибка: {resp.get("msg")}')
                return False
    ...
    def _checkConnect(self):
        ...
        try:
            resource = self.ses.post(f'{self.host}/panel/inbound/list/', data=self.data).json()
            if not resource.get('success'):
                logger.warning(f'🛑Ошибка при проверке подключения: {resource.get("msg")}')
                return False
            ...
            logger.error(f"Ошибка сети при _checkConnect: {e}")
            return False
    ...
    def _addNewConnect(self):
        ...
        cert_ok, cert_obj_or_msg = self._getNewX25519Cert()
        if not cert_ok:
        ...
        try:
            response = self.ses.post(f"{self.host}/panel/inbound/add", headers=header, json=data).json()
    ...
    def _getNewX25519Cert(self):
        ...
        try:
            response = self.ses.post(f"{self.host}/server/getNewX25519Cert", data=self.data).json()
            if response.get('success'):
                return (True, response['obj'])
            else:
    ...
    def addOrUpdateKey(self, vpn_key, isUpdate=False, isActiv=True, isIOS=False):
        """
        ...
        if not isUpdate:
            ...
            # /panel/inbound/updateClient/<vpn_key>
            command = f'/panel/inbound/updateClient/{vpn_key}'

        try:
            resource = self.ses.post(f'{self.host}{command}', headers=header, json=data).json()
    ...
    def deleteKey(self, vpn_key):
        ...
        try:
            ...
            response = self.ses.post(f"{self.host}/panel/inbound/1/delClient/{vpn_key}", data=self.data).json()
            if response.get('success'):
        ...
        try:
            ...
            for client in data_inb.get("clientStats", []):
                ...
                if str(client.get('enable', 'false')).lower() == 'true':
                    ...
                    url = self._getLink(vpn_key)  # Генерируем ссылку
...
if __name__ == "__main__":
    vless = VLESS(ip="212.34.140.237", password="gdCDV57RQut64Uzg")

    # Добавляем нового клиента
    ok, link_or_msg = vless.addOrUpdateKey('TEST_VPN_77777')
    if ok:
    ...
    # Удаляем ключ
    ok, msg = vless.deleteKey('TEST_VPN_77777')
    if ok:
